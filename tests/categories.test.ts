...
describe("categorias — Etapa 1", () => {
  ...
  describe("normalização de nome (bug MESADA PAPAI)", () => {
    it("impede duplicata por maiúscula", async () => {
      const first = await createCategory(householdId, { name: "Mesada Papai" });
      expect(first.status).toBe("created");

      const dup = await createCategory(householdId, { name: "MESADA PAPAI" });
      expect(dup.status).toBe("error");
    });

    it("impede duplicata por acento", async () => {
      const first = await createCategory(householdId, { name: "Educação" });
      expect(first.status).toBe("created");

      const dup = await createCategory(householdId, { name: "educacao" });
      expect(dup.status).toBe("error");
    });
  });
  ...
  describe("fusão de subcategoria com categoria existente", () => {
    ...
    it("reaproveita o mesmo id ao confirmar (não duplica lançamento nenhum)", async () => {
      const internet = await createCategory(householdId, { name: "Internet" });
      const carro = await createCategory(householdId, { name: "Carro" });
      if (internet.status !== "created" || carro.status !== "created") {
        throw new Error("setup failed");
      }

      const merged = await createCategory(householdId, {
        name: "Internet",
        parentId: carro.category.id,
        confirmMerge: true,
      });
    ...
    it("não permite subcategoria de subcategoria", async () => {
      const carro = await createCategory(householdId, { name: "Carro" });
      if (carro.status !== "created") throw new Error("setup failed");
      const sub = await createCategory(householdId, {
        name: "Combustível",
        parentId: carro.category.id,
      });
  ...
  describe("ordenação", () => {
    it("normal alfabética primeiro, depois reserve, depois awaiting_review por último", async () => {
      await ensureAwaitingReviewCategory(householdId);
      await createCategory(householdId, { name: "Zebra" });
      await createCategory(householdId, { name: "Alimentação" });
      await createCategory(householdId, { name: "Reserva de emergência", categoryType: "reserve" });
  ...
  describe("exclusão com lançamentos existentes", () => {
    it("pede decisão quando há lançamentos, e move ao decidir 'move'", async () => {
      const origem = await createCategory(householdId, { name: "Lazer" });
      const destino = await createCategory(householdId, { name: "Outros" });
      ...
      await pool.query(
        `INSERT INTO financial_entries (household_id, entry_type, entry_date, description, amount, category_id)
         VALUES ($1, 'expense', '2026-09-01', 'Cinema', 50.00, $2)`,
        [householdId, origem.category.id],
      );
    ...
    it("exclui os lançamentos junto ao decidir 'delete'", async () => {
      const origem = await createCategory(householdId, { name: "Assinaturas" });
      if (origem.status !== "created") throw new Error("setup failed");

      await pool.query(
        `INSERT INTO financial_entries (household_id, entry_type, entry_date, description, amount, category_id)
         VALUES ($1, 'expense', '2026-09-01', 'Streaming', 30.00, $2)`,
        [householdId, origem.category.id],
      );
    ...
    it("exclui direto sem perguntar quando não há lançamentos", async () => {
      const cat = await createCategory(householdId, { name: "Categoria Vazia" });
  ...
  describe("edição", () => {
    it("renomear passa pela mesma checagem de duplicata", async () => {
      const a = await createCategory(householdId, { name: "Categoria A" });
      const b = await createCategory(householdId, { name: "Categoria B" });
    ...
    it("atualiza cor sem exigir nome", async () => {
      const cat = await createCategory(householdId, { name: "Saúde" });
      if (cat.status !== "created") throw new Error("setup failed");
